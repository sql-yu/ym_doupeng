        ...
        <ul class="mailbox-attachments clearfix {{ $grid->formatTableClass() }} p-0"  id="{{ $tableId }}">
            @foreach($grid->rows() as $row)
                <li style="height: 200px;" >
                    <span class="mailbox-attachment-icon has-img" style="height: 102px;">
                        {!! $row->column('cate_image') !!}
                    </span>
                    <div class="mailbox-attachment-info" style="height: 92px;"  >
                        ...
                        <div class="d-flex justify-content-between item">
                            <span class="mailbox-attachment-name">{!! $row->column('game_cate_name') !!}</span> {!! $row->column('game_cate_name_cn') !!}
                        </div>
